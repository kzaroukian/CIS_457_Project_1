...
int main(int argc, char** argv) {
	...
	if (port < 1024) {
		port = 9999;
		printf("Port error... Defaulting to 9999.\n");
	}
	...
	while(1){
		...
		if(n < 0)
			...
			while (1) {
				...
				if ((client_response[0] == 'A'+last_ack_needed && strcmp(client_response+HEADER_SIZE, END_OF_FILE) == 0)
	...
	*(read_buffer) = (char)('A'+(*pack_ID)); // add the identifier to the beginning of the packet
	...
	if (diff == 0) {
		...
		*read_buffer = (char)('A'+(*pack_ID-1));
		...
		strcpy(read_buffer+HEADER_SIZE, END_OF_FILE);
		sendto(sockfd, read_buffer, sizeof(END_OF_FILE)+HEADER_SIZE, 0, (struct sockaddr*)&clientaddr, len);
		return 1;
	}
	if (diff <= PACKET_SIZE) {
		...
		fread(read_buffer+HEADER_SIZE, 1, PACKET_SIZE, file_ptr);
