...
int main(int argc, char** argv) {
	...
	while (1) {
		...
		if (packet_length < 0) {
			...
			if (strcmp(buffer+HEADER_SIZE, END_OF_FILE) == 0) {
				...
				// write to the file
				fwrite(buffer+HEADER_SIZE, 1, packet_length - HEADER_SIZE, file_out);
				...
				stored_packet_data_sizes[*buffer - 'A'] = packet_length - HEADER_SIZE;
